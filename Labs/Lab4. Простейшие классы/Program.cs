...
namespace lab4
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Консольный вывод задачи 7:");
            try
            {
                Rect myRect = new Rect(new double[8] { 1, 1, 1, 5, 15, 5, 15, 1 });
                float k = 5;
                myRect.SetScale(k);
                double[,] newVertices = myRect.GetVertices();
                Console.WriteLine($"Координаты вершин масшатбированного в {k} раз прямоугольника:");
                foreach (double vertex in newVertices)
                {
                    Console.Write(vertex + "\t");
                }
                myRect.GiveShareRect();
            }
            catch (Exception e) 
            {
                Console.WriteLine(e.Message);
